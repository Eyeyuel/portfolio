"use client";

import { motion } from "motion/react";
import { Card, CardDescription } from "@/components/ui/card";
import Image from "next/image";
import Title from "@/components/title";
import { DotSquare } from "@/components/dot-square";

// Replace the static content with this animated version
export default function AboutPageContent() {
  // Stagger container for the text paragraphs
  const containerVariants = {
    hidden: {},
    visible: {
      transition: {
        staggerChildren: 0.1,
        delayChildren: 0.2,
      },
    },
  };

  const itemVariants = {
    hidden: { opacity: 0, y: 25 },
    visible: {
      opacity: 1,
      y: 0,
      transition: {
        type: "spring" as const,
        stiffness: 100,
        damping: 12,
      },
    },
  };

  return (
    <div>
      {/* Title – already animated by your Title component */}
      <Title symbol="/">about-me</Title>

      {/* Subheading */}
      <motion.p
        initial={{ opacity: 0, y: 15 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true, amount: 0.3 }}
        transition={{
          type: "spring",
          stiffness: 100,
          damping: 12,
          delay: 0.1,
        }}
        className="text-card-foreground"
      >
        Who am i?
      </motion.p>

      <div className="mt-20">
        <div className="w-full">
          <Card className="flex flex-row justify-between bg-transparent border-none shadow-none">
            {/* left side – staggered text paragraphs */}
            <motion.div
              variants={containerVariants}
              initial="hidden"
              whileInView="visible"
              viewport={{ once: true, amount: 0.2 }}
              className="flex flex-col items-start justify-between"
            >
              <motion.div variants={itemVariants}>
                <CardDescription>Hello, i&apos;m Eyuel!</CardDescription>
              </motion.div>

              <motion.div variants={itemVariants}>
                <CardDescription>
                  I’m a full‑stack developer with experience building modern,
                  scalable, and user‑friendly web applications using both
                  frontend and backend technologies. I develop responsive
                  applications from scratch and turn ideas into reliable digital
                  solutions with clean and efficient system design.
                </CardDescription>
              </motion.div>

              <motion.div variants={itemVariants}>
                <br />
                <CardDescription>
                  Building software and solving real‑world problems through
                  technology is a strong passion of mine. I enjoy helping
                  clients create effective web solutions while continuously
                  improving my skills and staying up to date with modern
                  technologies and development practices.
                </CardDescription>
              </motion.div>
            </motion.div>

            {/* right side – image with spring scale‑in */}
            <motion.div
              initial={{ opacity: 0, scale: 0.85 }}
              whileInView={{ opacity: 1, scale: 1 }}
              viewport={{ once: true, amount: 0.3 }}
              transition={{
                type: "spring",
                stiffness: 150,
                damping: 15,
                delay: 0.2,
              }}
              className="hidden md:block basis-[50%] relative"
            >
              <Image
                width={300}
                height={200}
                alt="face image"
                src="./vercel.svg"
              />

              {/* DotSquares already animated via their own components */}
              <DotSquare
                gap={3}
                row={4}
                className="absolute top-[30%] right-[20%] grid-cols-5"
              />
              <DotSquare
                gap={2}
                className="absolute top-0 left-0 grid-cols-5"
              />
            </motion.div>
          </Card>
        </div>
      </div>
    </div>
  );
}
